...
##判定①　位置も数字も正解
    for i in 0..3
      if ary1[i] == ary2[i]
        d.flag1 += 1
        d.kensa(ary1,i) {|x,y| x[y] = Mastermind::MASK[y]}
        d.kensa(ary2,i) {|x,y| x[y] = Mastermind::MASK[y]}
      end
    end
   #print "ary1=",ary1,"ary2=",ary2,"\n"

##判定②　位置のみ正解
      ary1.each.with_index do |item1,i1|
        ary2.each.with_index do |item2,i2|
          if i1 != i2
            if item1 == item2
              d.flag2 += 1
              d.kensa(ary1,i1) {|x,y| x[y] = Mastermind::MASK[y]}
              d.kensa(ary2,i2) {|x,y| x[y] = Mastermind::MASK[y]}
            end
          end
        end
      end
